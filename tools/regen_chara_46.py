...
TARGET = BASE_DIR / 'img/Chapter8/chara/image_merge_order_chara_46.png'
...
CHAR_PROMPT = (
    f"{CHAR_STYLE}, "
    "young Japanese woman, 23 years old, long straight black hair with natural flow, pale fair skin, slender build, "
    "thin delicate oval metal-frame glasses, gentle and slightly melancholic expression, defined facial features, "
    "brown cardigan over white inner shirt, layered casual style, "
    "quiet and delicate appearance, reserved and sensitive personality"
)
...
def add_top_padding(ratio=0.12):
    """生成画像の上部に白余白を追加して頭の切れを防ぐ"""
...
def fit_character_to_canvas(scale=0.95, top_margin=20):
    """キャラを縮小して上余白固定で配置 → 500×500 白背景で保存"""
    from PIL import Image
    TARGET_SIZE = 500
    img = Image.open(TARGET).convert('RGB')
